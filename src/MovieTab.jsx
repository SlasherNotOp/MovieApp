...
const MovieTab = () => {
    ...
    const fetchMovie = async () => {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/movie/${id}?api_key=${import.meta.env.VITE_TMDB_API_KEY}&include_video=true&language=en-US`
        );
  ...
  return (
    ...
    <img src={img_base_path + movie.backdrop_path} alt="" className=' object-cover w-full h-screen' />
      ...
      <div className="single_page absolute w-full h-screen flex justify-around ">
        ...
        <div className="single_con w-2/3 flex flex-col gap-6 items-start justify-center text-white px-5">
          ...
          <div className=" type flex flex-wrap gap-2">
            <p className="text-lg flex gap-2">
              <strong>Type:</strong> 
          ...
          <div className="production flex w-full gap-5 mt-3 flex-wrap">
            {movie.production_companies.map((production, index) => (
              <div key={index} className="flex flex-col items-center gap-2">
                <div className=" productionImg w-24 h-12 overflow-hidden bg-white p-2 rounded-md">
                  <img
                    src={production.logo_path ? img_base_path + production.logo_path : ""}
                    alt={production.name}
